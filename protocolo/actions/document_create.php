...
$nivelSigilo = trim((string)($_POST['nivel_sigilo'] ?? ''));
...
if ($confidencial && $nivelSigilo === '') {
    $_SESSION['flash_error'] = 'Informe o nível de sigilo.';
    header('Location: ../index.php');
    exit;
}
...
try {
    ...
    if (is_array($destUsuarios)) {
        $stmt = $conn->prepare('INSERT INTO doc_destinatarios (documento_id, tipo_destino, ordem, usuario_destino) VALUES (?, ?, ?, ?)');
        $ordem = 1;
        foreach ($destUsuarios as $usuarioId) {
            $usuarioId = (int)$usuarioId;
            if ($usuarioId > 0) {
                $tipo = 'interno';
                $stmt->bind_param('isii', $documentoId, $tipo, $ordem, $usuarioId);
                $stmt->execute();
                $ordem++;
            }
        }
    ...
    if (is_array($destUnidades)) {
        $stmt = $conn->prepare('INSERT INTO doc_destinatarios (documento_id, tipo_destino, ordem, id_unidade_destino) VALUES (?, ?, ?, ?)');
        $ordem = 1;
        foreach ($destUnidades as $unidadeId) {
            $unidadeId = (int)$unidadeId;
            if ($unidadeId > 0) {
                $tipo = 'interno';
                $stmt->bind_param('isii', $documentoId, $tipo, $ordem, $unidadeId);
                $stmt->execute();
                $ordem++;
            }
        }
    ...
    if (is_array($destExternos)) {
        $stmt = $conn->prepare('INSERT INTO doc_destinatarios (documento_id, tipo_destino, ordem, nome_externo, orgao_externo, email_externo, endereco_externo) VALUES (?, ?, ?, ?, ?, ?, ?)');
        $ordem = 1;
        foreach ($destExternos as $externo) {
            ...
            $tipo = 'externo';
            $stmt->bind_param('isissss', $documentoId, $tipo, $ordem, $nome, $orgao, $email, $endereco);
            $stmt->execute();
            $ordem++;
        }
        $stmt->close();
